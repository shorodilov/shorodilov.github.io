        ...
        <div className="max-w-page space-y-section mx-auto">
            <header className="max-w-readable space-y-2">
                ...
                <p className="type-body text-foreground-muted">
                    Composite roles bundle family, weight, size, line height, and intentional casing into a stable
                    utility contract.
                </p>
            </header>
            ...
            <section aria-label="Typography roles" className="space-y-4">
                {typographyRoles.map((role) => (
                    <article
                        className="rounded-card border-border bg-surface grid gap-4 border p-5 md:grid-cols-3 md:items-center"
                        key={role.className}
                    >
                        ...
                        <p className={`${role.className} text-foreground break-words md:col-span-2`}>{sample}</p>
                    </article>
                ))}
            </section>
        </div>
