...
const ColorSwatch = ({ label, source, swatchClassName, swatchVariable, value }: ColorSwatchData) => (
  <article className="overflow-hidden rounded-card border border-border bg-surface">
    <div
      aria-hidden="true"
      className={`h-24 border-b border-border ${swatchClassName ?? ""}`}
    ...
    <div className="space-y-1 p-4">
      ...
      <p className="type-caption break-all text-foreground-muted">{source}</p>
    ...
    <div className="mx-auto max-w-page space-y-section">
      <header className="max-w-readable space-y-2">
        ...
        <p className="type-body text-foreground-muted">
          Reference values remain stable. Semantic utilities respond to the selected Storybook theme and form the
          public color contract used by the interface.
        </p>
      </header>
