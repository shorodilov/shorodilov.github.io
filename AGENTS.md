...
from repository state, user direction, issues, or planning documents. If the
stage affects a decision and is unclear, ask before proceeding.
...
- The project AI rules listed below for assistant behavior.
...
- Do not generate code, documents, diagrams, configs, copy, or snippets unless
  the user explicitly asks with an action verb such as `create`, `generate`,
...
- Use Tailwind for styling new UI unless the surrounding code clearly uses a
  different local pattern.
...
Avoid marketing-heavy copy, decorative complexity without purpose, and design
choices that make the portfolio harder to maintain.

## Change Workflow

1. Read `.ai/rules/*` and the relevant project files.
2. Identify the current stage from available context when giving broad guidance.
